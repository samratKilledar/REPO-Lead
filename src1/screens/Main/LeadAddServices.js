...
import { fetchServices } from '../../redux/actions/AddserviceActions'; // Import the fetchServices action
...
const LeadAddService = () => {
  ...
  const { services: dropdownServices } = useSelector((state) => state.Addservice);

  // State to hold formatted services for the dropdown
  ...
  // Format dropdownServices into the required format for the dropdown
  useEffect(() => {
    if (dropdownServices?.length) {
      const formattedData = dropdownServices.map((service) => ({
        label: service.value01, // Use `value01` for the label
        value: service.value01, // Use `value01` for the value
      }));
      setFormattedServices(formattedData);
    }
  }, [dropdownServices]);
    ...
    <View style={styles.container}>
      ...
      <View style={styles.centerContainer}>
        ...
        <Dropdown
          ...
          selectedValue={services}
          onValueChange={setService}
          options={formattedServices} // Use formattedServices for the dropdown options
          zIndex={2000}
        />
        <CustomButton title="Submit" customStyle={{ width: -30 }} textStyles={styles.nextButtonText} />
      </View>
